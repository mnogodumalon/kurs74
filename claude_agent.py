...
async def main():
    ...
    options = ClaudeAgentOptions(
        ...
        # allowed_tools set below based on mode
        cwd="/home/user/app",
    ...
    # ============================================================
    # ALLOWED TOOLS — mode-dependent
    # ============================================================
    # UI-First: NO Read tool. Agent gets ALL file contents from generate_typescript
    # response + its own Write outputs are in conversation history. Read triggers
    # the destructive Write→Read→Write pattern (wastes 40-90s per file).
    # Also no Task/TodoWrite — agent should code immediately, not plan.
    mcp_tools = [
    ...
    if ui_first_mode:
        options.allowed_tools = ["Bash", "Write", "Edit", "Glob", "Grep"] + mcp_tools
        print("[LILO] UI-First-Mode: Read tool DISABLED (all files in context)")
    else:
        options.allowed_tools = ["Bash", "Write", "Read", "Edit", "Glob", "Grep", "Task", "TodoWrite"] + mcp_tools
